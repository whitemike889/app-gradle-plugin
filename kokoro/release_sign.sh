...
LAST_BUILD=$(ls prod/app-gradle-plugin/gcp_ubuntu/release/ | sort -rV | head -1)

# find the jars and the pom in the latest build artifact directory
FILES=$(find `pwd`/prod/app-gradle-plugin/gcp_ubuntu/release/${LAST_BUILD}/* -type f \( -iname \*.jar -o -iname \*.pom \))
